...
const ChatInterface = ({ recipientId }) => {
  ...
  const roomName =
    userDetails.role === "Delivery Driver"
      ? "DeliveryDriversRoom"
      : getSortedRoomName(userDetails.id, recipientId);

  const loadChatHistory = async () => {
    try {
      const response = await fetch(
        `https://delivery-it-server.onrender.com/getMessages?roomName=${roomName}`
      );
      if (response.ok) {
    ...
    try {
      ...
      if (!response.ok) {
        const createResponse = await fetch(
          "https://delivery-it-server.onrender.com/createRoom",
          {
            method: "POST",
            headers: {
    ...
    if (text.trim()) {
      ...
      try {
        ...
        const sendMessageResponse = await fetch(
          "https://delivery-it-server.onrender.com/sendMessage",
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
        ...
        if (sendMessageResponse.ok) {
          ...
          console.error(
            ...
            await sendMessageResponse.text()
          );
        }
      } catch (error) {
